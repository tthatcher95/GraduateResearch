...
def get_hashtag_users_df(df):
        hashtag_users_df = df.loc[(df['User_Mentions'] != '[]')]
...
def build_mentions_dict(df):
    ...
    def make_jaccard_csv(user_hashtag_dict):
        with open('jaccard_hashtags.csv', 'w') as csv_file:
            writer = csv.writer(csv_file)
            for key, value in user_hashtag_dict.items():
                writer.writerow([key, value])
    ...
    domain_list = list(zip(df['User_Mentions'], df['User_ID']))
    ...
    hash_df = pd.read_csv("user_hashtag.csv", names=['User_Mentions', 'Count'])
    hash_df.dropna(inplace=True)
    jaccard_domain_dict = {}

    for pair in itertools.product(hash_df['User_Mentions'][2:], repeat=2):
